...
## Move the build-time `resolve.conditions`/`mainFields` overrides into `configEnvironment`

`packages/run/src/vite/plugin.ts` › `plugin` (the `config` hook, `if (isBuild)` block) | 2026-08-03 | impact:med | effort:low

The plugin sets `pluginConfig.resolve.conditions` and `pluginConfig.resolve.mainFields` at the top level, branching on `isSSRBuild` (`"node"`/`[]` vs `"browser"`/`["browser"]`). Under Vite 8's environment API those two keys are stripped for every non-client environment: `getDefaultEnvironmentOptions` builds non-client defaults as `{...config.resolve, mainFields: void 0, conditions: void 0}`, while only `defaultClientEnvironmentOptions` keeps `resolve: config.resolve` intact. Verified by `resolveConfig`-ing the `micro-frame-fetch` fixture with the plugin and a `build.ssr` entry: `environments.ssr.resolve.conditions` is Vite's untouched server default `["module","node","development|production"]`, and the plugin's list lands on `environments.client` — the environment that SSR build never uses. So the `isSSRBuild` branches are dead, and the SSR build silently runs on Vite defaults (which notably _do_ include `"module"`, unlike the plugin's client list). Set these per-environment from the existing `configEnvironment` hook instead, so the SSR-targeted values actually apply — and decide deliberately whether the client build should keep dropping `"module"`.
