...
- Schema-validated values are `[value, issues]` pairs: `const [search, searchIssues] = ctx.search`, `const [params, paramIssues] = ctx.params`, `const [body, issues] = await ctx.body`. When validation fails `issues` is set (and `value` is the raw input); handle it before use (reject, or re-render the page as in the POST above). Validators run once, on first access, and rendering a page can access both `params` and `search`, so expect them to run for every page request; a throwing validator fails the request and its value then reads `undefined` (not reflected in its type), so check both in `+500.marko`. Without one, `ctx.params`/`ctx.search` are plain objects; read them rather than `ctx.url.searchParams` or `ctx.request.formData()`.
