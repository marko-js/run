...
describe("persisted client navigate()", () => {
  ...
  it("accepts a metadata-only frame as an applied patch", async () => {
  ...
  it("applies an opaque prefix delta", async () => {
    ...
    assert.equal(state.have, "shared-new");
  });
  ...
  it("hands the applied prefix's state to an immediately following navigation", async () => {
    ...
    const stalled = streamedPatch("http://localhost:3000/item/2");
    fetchImpl = async () => stalled.response;
    const state = makeState({ have: "origin-token" });
    const first = run(
      state,
      "http://localhost:3000/item/2",
      makeEntry(applied, (source) =>
        source === "fill-a1" ? "~=hybrid-token" : true,
      ),
      { fallbacks },
    );
    await flush();
    ...
    assert.equal(headers["x-marko-have"], "hybrid-token");
    assert.equal(state.appliedUrl, "/item/3");
    assert.equal(window.location.pathname, "/item/3");
    assert.deepEqual(fallbacks, []);
  });
...
describe("persisted client possession token initialization", () => {
  ...
  // Later suites capture `appliedUrl` at register time; restore the URL the
  // navigations in this suite pushed.
  afterEach(() => {
    window.history.replaceState(null, "", "/list?page=1");
  });

  it("reads the document token once and honors an explicit-empty replacement", async () => {
    const { register } = await import("../runtime/persisted.js?token-once");
    let haveReads = 0;
    let patchResult: (source: string) => true | string = () => true;
    const entry: PersistedEntry = {
      patch: () => (source) => patchResult(source),
    };
    register(
      () =>
        Promise.resolve(
          () => [2, () => Promise.resolve(entry)] satisfies RouteEntry,
        ),
      1,
      "build-1",
      () => {
    ...
    // First navigation carries the initial document's token, and its patch
    // replaces the store with the canonical explicit-empty token ("~=").
    patchResult = () => "~=";
    fetchImpl = async () =>
      patchResponse(["fill"], "http://localhost:3000/item/2");
    assert.equal(click(link("/item/2")), true);
    await flush();
    assert.equal(haveReads, 1);
    assert.equal(
      (fetchCalls[0].init.headers as Record<string, string>)["x-marko-have"],
      "doc-token",
    );

    // The empty store is canonical: the next navigation must send no token
    // rather than resurrecting the initial document's stale one.
    fetchCalls = [];
    patchResult = () => true;
    fetchImpl = async () =>
      patchResponse(["fill"], "http://localhost:3000/item/3");
    assert.equal(click(link("/item/3")), true);
    await flush();
    assert.equal(haveReads, 1);
    assert.equal(fetchCalls.length, 1);
    assert.equal(
...
describe("persisted client possession token read failure", () => {
  useBrowserGlobals(true);
  beforeEach(() => {
    fetchCalls = [];
    navigations.length = 0;
  ...
  it("falls back to native navigation when the token read throws", async () => {
    const { register } = await import("../runtime/persisted.js?token-throws");
    const loadMatch = () =>
      Promise.resolve(
        () =>
          [2, () => Promise.resolve(makeEntry([]))] satisfies RouteEntry as
            | RouteEntry
            | undefined,
      );
    register(loadMatch, 1, "build-1", () => {
      // The generated accessor dereferences the runtime global, which is
      // missing when its inline script never ran.
      throw new TypeError("missing runtime global");
    });

    // The click is already prevented, so a dead accessor must not strand it.
    assert.equal(click(link("/item/2")), true);
    await flush();
    assert.equal(fetchCalls.length, 0);
    assert.deepEqual(navigations, ["assign:http://localhost:3000/item/2"]);

    // A failed read does not latch: once readable (e.g. after a later
    // register) the token flows again.
    navigations.length = 0;
    fetchImpl = async () =>
      patchResponse(["fill"], "http://localhost:3000/item/2");
    register(loadMatch, 1, "build-1", () => "doc-token");
    assert.equal(click(link("/item/2")), true);
    await flush();
    assert.deepEqual(navigations, []);
    assert.equal(fetchCalls.length, 1);
