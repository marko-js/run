...
describe("persisted render() possession echo (x-marko-have)", () => {
  ...
  it("passes the echo through opaquely for a patch render", () => {
  ...
  it("selects source and target descriptors for a cross-route patch", () => {
    ...
    assert.equal(persisted.patch.have, "P-token");
  ...
  it("does not honor the echo on a non-patch persisted render", () => {
    assert.equal(
      renderPersisted(undefined, { "x-marko-have": "P-token" }).patch?.have,
      undefined,
    );
  });

  it("drops have when the valid source route has no descriptor", () => {
    const persisted = renderPersisted("1", { "x-marko-have": "P-token" }, [
      undefined,
      undefined,
      "target descriptor",
    ]);
