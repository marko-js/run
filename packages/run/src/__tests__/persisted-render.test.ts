...
describe("persisted render() response headers", () => {
  it("patch render (default init) advertises the patch content-type", () => {
    ...
    assert.equal(
      ...
      "accept, x-marko-echo, x-marko-claim-set, x-marko-build, x-marko-route, x-marko-from",
    );
  ...
  it("patch render preserves a caller's own vary tokens", () => {
    ...
    assert.equal(
      res.headers.get("vary"),
      "cookie, accept, x-marko-echo, x-marko-claim-set, x-marko-build, x-marko-route, x-marko-from",
    );
  });
...
describe("persisted request negotiation", () => {
  ...
  it("holds the manifest's shells for the target route only", () => {
    globalThis.__MARKO_RUN_SHELLS__ = { "2": ["a1", "j5"], "1": ["b2"] };
    try {
      let options: any;
      negotiate(createPatchRequestHeaders(2, 1, "current-build")).ctx.render(
        {
          render: (_input: unknown, next: unknown) => (
            (options = next),
            rendered
      ...
      const { heldShells } = options.persisted.patch;
      assert.equal(heldShells("a1"), true);
      assert.equal(heldShells("j5"), true);
      assert.equal(heldShells("b2"), false, "another route's floor");
      assert.equal(heldShells("unknown"), false);
    } finally {
      delete globalThis.__MARKO_RUN_SHELLS__;
    }
  });

  it("ships full shells when no manifest was delivered", () => {
    let options: any;
    negotiate(createPatchRequestHeaders(2, 1, "current-build")).ctx.render(
      {
        render: (_input: unknown, next: unknown) => (
    ...
    assert.equal(options.persisted.patch.heldShells, undefined);
  });
  ...
  it("rejects a stale build as a non-cacheable non-patch response", () => {
    ...
    assert.equal(mismatch?.status, 409);
    assert.equal(mismatch?.headers.get("cache-control"), "private, no-store");
    assert.equal(
      mismatch?.headers.get("vary"),
      "accept, x-marko-echo, x-marko-claim-set, x-marko-build, x-marko-route, x-marko-from",
    );
