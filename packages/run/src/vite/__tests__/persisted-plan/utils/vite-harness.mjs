...
async function loadPreC0Plugin() {
  ...
  const baselineRef = process.env.G3_BASELINE_REF || "g3-baseline-pre-c0";
  const baselineSha =
    process.env.G3_BASELINE_SHA || "b6bd355d897aa249866118ff16be4e4ed8fb8d49";
  execFileSync(
    "git",
  ...
  for (const ref of [baselineRef, baselineSha]) {
    try {
      execFileSync("git", ["-C", baselineRoot, "checkout", "--detach", ref], {
        stdio: "ignore",
      });
      checkedOut = true;
      break;
