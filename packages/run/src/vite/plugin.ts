...
export default function markoRun(opts: Options = {}): Plugin[] {
  ...
  async function writeEntryTemplate(
    ...
    const source = renderRouteTemplate(
      route,
      assertPersistedMarkoApi(await getMarkoApiForRoute(context, route), route),
      !isBuild,
      persistedRoute,
    );
    ...
    return (renderVirtualFilesResult ??= (async () => {
      try {
        ...
        for (const route of routes.list) {
          ...
          if (route.templateFilePath) {
            await writeEntryTemplate(context, route, persisted);
          }
