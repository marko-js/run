...
export function renderRouteEntry(route: Route, rootDir: string): string {
  ...
  if (page) {
    ...
    imports.writeLines(`import page from "${templatePath}";`);
  }
  if (meta) {
...
export function renderRouter(
  ...
  if (options.persisted) {
    // Sparse truthy entries mark the route indexes that are persisted pages.
    const pageRoutes = routes.list.filter((route) => route.page);
    const persistedRoutes = Array<string>(
      Math.max(-1, ...pageRoutes.map((route) => route.index)) + 1,
    );
    for (const route of pageRoutes) {
      persistedRoutes[route.index] = "1";
    }
    writer.writeLines(`const persisted = [${persistedRoutes.join(",")}];`);
  }
...
export interface RoutesClientArtifacts {
  routes: {
    routeIndex: number;
...
export function renderRoutesClient(
  routes: BuiltRoutes,
  rootDir: string,
  artifacts?: RoutesClientArtifacts,
): string {
  const writer = createStringWriter();
  ...
  const artifactRoutes = artifacts
    ? new Map(artifacts.routes.map((route) => [route.routeIndex, route]))
    : undefined;
  if (artifacts) {
    const registry = pageRoutes.map((route) => {
      const artifactRoute = artifactRoutes!.get(route.index);
      if (!artifactRoute) {
        throw new Error(
          `@marko/run: route emission is incomplete for route ${route.index}`,
        );
      }
      return artifacts.artifacts[artifactRoute.primaryRegistryIndex];
    });
    writer.writeLines(
      `export const artifactRegistry = [${registry
        .map((id) => (id ? `()=>import(${JSON.stringify(id)})` : "0"))
        .join(",")}];`,
      `export const capability = ${JSON.stringify(artifacts.capability)};`,
    );
  }
  writer.writeBlockStart("const routes = [");
  for (const route of pageRoutes) {
    const artifactRoute = artifactRoutes?.get(route.index);
    if (artifacts && !artifactRoute) {
      throw new Error(
        `@marko/run: route emission is incomplete for route ${route.index}`,
      );
    }
    writer.writeLines(
      artifactRoute
        ? `[${route.index}, ${renderClientRouteReg(route)},${JSON.stringify(
            artifactRoute.possessionShellIndexes,
          )}],`
        : `[${route.index}, () => import(${JSON.stringify(
            `${normalizedRelativePath(rootDir, route.templateFilePath!)}?persisted`,
          )}), ${renderClientRouteReg(route)}],`,
    );
  }
  writer.writeBlockEnd("];");
  writer.writeLines(
    artifacts
      ? "export default Object.assign((pathname) => { const i = routes.findIndex((route) => route[1].test(pathname)); if (i < 0) return; const route = routes[i]; return [route[0], artifactRegistry[i], route[1], route[2]]; }, { artifactRegistry, capability });"
