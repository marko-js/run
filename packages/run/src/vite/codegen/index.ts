...
export function renderRouteTemplate(
  ...
  // route matcher (the same trie the server ranks with, whose terminals are
  // route-entry tuples: lazy loaders for each route's template module and
  // `?update` entry — same-route navigations load only this route's entry
  // chunk; cross-route navigations also load the target's code), this
  // route's own build-stable index, and the build identity (serialized
  // under the internal "~run" global). Special (404/500) pages have no
  // route to navigate within, and the class API has no persisted support.
  if (
    persisted &&
    markoApi !== "class" &&
...
function match_internal(method, pathname) {
  ...
  if (options.persisted) {
    ...
    // the route the client matched is the one this URL resolves to (the
    // client runs the same generated trie, so within a build this only
    // diverges when a redirect moved the request to another route) AND the
    // client's build matches this server's (route indexes, compiled
    // accessors, and register ids are only stable within one build) —
    // otherwise a 409 tells the client router to fall back to a full
    // navigation.
    writer.writeLines(
...
// dynamics per level, catch-alls last). `terminal` renders the expression
// returned for a matched route -- the server router returns handler match
// objects, the persisted client table returns route-entry tuples -- so both
// matchers are the same generated logic and cannot disagree on ranking.
...
function renderMatch(
  ...
  // identity: persisted update negotiation compares it against the client's
  // `x-marko-route`/`x-marko-from` headers (route indexes are stable within a
  // build, and the build hash is verified alongside). Non-persisted builds
  // omit it so their router output stays byte-identical.
 ...
 * server ranks with (page routes only), whose terminals are route-entry
 * tuples — the route's build-stable index (sent as `x-marko-route`) plus
 * lazy loaders for the route's generated template module (importing it
 * registers the route's renderers, signals, and update merges) and its
 * `?update` entry (the compiled merge functions). Sharing the emitter with
 * the server router means client and server cannot disagree on ranking
 * within a build, and no path patterns or regex machinery ship to the
 * client at all.
 */
