...
// A page frozen mid-stream keeps its abort controller and half-applied
// DOM; the restore must not let that stream keep writing into a document
// the browser resurrected.
function onPagehide() {
...
function onPageshow(ev: PageTransitionEvent) {
  ...
  // A restored page that was mid-apply has DOM the committed claims no
  // longer describe, and no way to finish the superseded stream: reload
  // it rather than patch from a state neither side can name.
  if (state.partialApply) {
    location.reload();
    return;
  }
  // Otherwise the DOM is whole; only the URL may have moved under the
  // restore, and possessions re-snapshot live on the next request.
  state.appliedUrl = location.pathname + location.search;
}
